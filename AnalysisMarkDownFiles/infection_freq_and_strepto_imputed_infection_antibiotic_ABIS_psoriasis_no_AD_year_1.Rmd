---
title: "Infections and antibiotics: ABIS-psoriasis year-1 excluding auto-immune diseases"
...
output_dir <- paste0(path,"/infection_antibiotic_year_wise/with_overall_freq_imputation_infection_antibiotic_1_year_revision")
...
source(paste0(src_path, "/all_parameters_infection_antibiotics_ABIS_year_1.R"))
```
...
save ABISnr who participated in year-1
```{r}
participants_ABISnr <- (abis_table %>% dplyr::filter(par_1==1))$ABISnr
```
...
abis_table_labels <- 
  data.frame("Name" = Name, "Label" = Label)
...
```{r}
abis_table_original_infection_txt <- abis_table[,c("ABISnr", "b_12txt")]
...
names(attr(abis_table$b_42g, "labels")) <- c("Never", "1-2 times", "3-5 times", "6 times or more")

abis_table <- abis_table %>% 
  dplyr::mutate(
    child_dob = as.Date(as.yearmon(as.character(barn_år_mån), format = "%Y%m")),
    infection_frequency = pmax(b_42a, b_42b, b_42c, b_42d, b_42e, 
                               b_42f, b_42g, na.rm = TRUE) 
    ) %>%
  dplyr::select(-c(b_42e, b_42f, b_42g))
```
...
psoriasis_data_full <- psoriasis_data_full %>%
 ...
 dplyr::mutate(
  ...
  time = ifelse(
    par_1 != 1,
    NA,
    ifelse(is.na(ageC_1),1.00,round(ageC_1/12,2))),
  event =  ifelse(
    par_1 != 1,
    NA,
    ifelse(
...
```{r}
psoriasis_data_full <- psoriasis_data_full %>%
  dplyr::select(-all_of(categorical_explanatory_vars)) 
...
```{r}
print(names(psoriasis_data_full)[c(3:14,19:20)])
```
...
```{r}
for (voi in names(psoriasis_data_full)[c(3:14,19:20)]) {
...
all_parameters <- all_parameters %>% 
  dplyr::filter(!(category_variable %in% c("Measles", "Mumps", "Rubella")))
```
...
print("variables with more than 50% missing values")
print(P[P>0.5])
```

drop factors with over 50% missing values 
...
haven::write_sav(data = psoriasis_data_full,
                 path = paste0(output_dir, "/original_cox_data_year_1.sav"))
```
...
pred[,"ABISnr"] <- 0
pred[c("Common_cold_dichotomous_cat", "Gastroenteritis_dichotomous_cat",
      "Infection_requiring_penicillin_dichotomous_cat",
      "Influenza_dichotomous_cat"),"Infection_frequency_cat"] <- 0
...
long$Streptococcal_infections <- with(long, ifelse(is.na(Otitis)|is.na(Infection_requiring_penicillin_dichotomous_cat),
                                  NA,
                                  ifelse(
                                    ((Otitis=="Yes")&(Infection_requiring_penicillin_dichotomous_cat == "At least once")),
                                    "At least once","Never")))

long$Streptococcal_infections <- with(long, factor(Streptococcal_infections, levels = c("Never","At least once")))

long$Influenza_or_other_infection <- with(long, ifelse(is.na(Other_infection)&is.na(Influenza_dichotomous_cat),
                                  NA,
                                  ifelse(
                                    ((Other_infection=="Yes")|(Influenza_dichotomous_cat == "At least once")),
                                    "Yes","No")))

long$Influenza_or_other_infection <- with(long, factor(Influenza_or_other_infection, levels = c("No","Yes")))


long$Respiratory_infections_WCIP <- with(long, ifelse((is.na(Whooping_cough)) &
                                                     (is.na(Common_cold_dichotomous_cat)) &
                                                     (is.na(Influenza_dichotomous_cat)) & (is.na(Pneumonia)),
                                                   NA,
                                                   ifelse(((Whooping_cough == "Yes") |
                                                             (Common_cold_dichotomous_cat == "At least once") |
                                                             (Influenza_dichotomous_cat == "At least once") |
                                                             (Pneumonia == "Yes")
                                                   ), "At least once", "Never")
))
...
long$Respiratory_infections_WCP <- with(long, ifelse((is.na(Whooping_cough)) &
                                                     (is.na(Common_cold_dichotomous_cat)) &
                                                     (is.na(Pneumonia)),
                                                   NA,
                                                   ifelse(((Whooping_cough == "Yes") |
...
```{r warning=FALSE}
variables2test <- names(original_psoriasis_data_full)[c(3:11,23:24,25:26,12:22)]
...
```{r warning=FALSE}
variables2test <- names(original_psoriasis_data_full)[c(3:11,23:24,25:26,12:16)]
...
```{r}
variables2test <- names(imp_parallel$data)[c(3:11,23:24,25:26,12:22)]
...
``` {r}
variables2test <- names(imp_parallel$data)[c(3:11,23:24,25:26,12:16)]
...
filename <-paste(output_dir, "A_cc_univariate_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
...
filename <-paste(output_dir, "B_cc_univariate_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
filename <-paste(output_dir, "A_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- full_imputed_univariate_regression_table[-c(56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_imp_univariate_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_imp_univariate_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- full_imputed_multivariable_regression_table[-c(56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
variables2test <- names(original_psoriasis_data_post_puberty)[c(3:11,23:24,25:26,12:16)]
...
fit multivariable logistic regression models imputed post puberty data
``` {r warning=FALSE}
variables2test <- names(imp_parallel_post_puberty$data)[c(3:11,23:24,25:26,12:16)]
...
forest_plot <- forest_from_table(
  df2plot = df2plot,
  title = " Year 1 post puberty complete-case: multivariable regression model",
  univariate = FALSE, imputed_output = FALSE)

filename <-paste(output_dir, "post_puberty_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r, message=FALSE, warning=FALSE}
df2plot <- post_puberty_imputed_multivariable_regression_table[-c(34:39,40:42,56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "post_puberty_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
...
```{r, message=FALSE, warning=FALSE}
variables2test <- names(original_psoriasis_data_pre_puberty)[c(3:11,23:24,25:26,12:16)]
...
``` {r, message=FALSE, warning = FALSE}
variables2test <- names(imp_parallel_pre_puberty$data)[c(3:11,23:24,25:26,12:16)]
...
forest_plot <- forest_from_table(
  df2plot = df2plot[c(1:one_half),],
  title = " Year 1 pre puberty complete-case: multivariable regression model",
  univariate = FALSE, imputed_output = FALSE)

filename <-paste(output_dir, "A_pre_puberty_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_pre_puberty_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
```{r}
df2plot <- pre_puberty_imputed_multivariable_regression_table[-c(25:27,56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_pre_puberty_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_pre_puberty_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
```{r warning=FALSE}
variables2test <- names(original_psoriasis_data_girls)[c(3:11,23:24,25:26,12:16)]
...
``` {r warning=FALSE}
variables2test <- names(imp_parallel_girls$data)[c(3:11,23:24,25:26,12:16)]
...
forest_plot <- forest_from_table(
  df2plot = df2plot[c(1:one_half),],
  title = " Year 1 girls complete-case: multivariable regression model",
  univariate = FALSE, imputed_output = FALSE)

filename <-paste(output_dir, "A_girls_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_girls_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- girls_imputed_multivariable_regression_table[-c(56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_girls_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_girls_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
```{r warning=FALSE}
variables2test <- names(original_psoriasis_data_boys)[c(3:11,23:24,25:26,12:16)]
...
``` {r}
variables2test <- names(imp_parallel_boys$data)[c(3:11,23:24,25:26,12:16)]
...
forest_plot <- forest_from_table(
  df2plot = df2plot,
  title = " Year 1 boys complete-case: multivariable regression model",
  univariate = FALSE, imputed_output = FALSE)

filename <-paste(output_dir, "boys_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- boys_imputed_multivariable_regression_table[-c(34:39,40:42,56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_boys_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_boys_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
```{r}
# variables2test <- names(original_psoriasis_data_severe)[c(3:11,23:24,12:16)]
# 
...
``` {r warning=FALSE}
variables2test <- names(imp_parallel_severe$data)[c(3:11,23:24,25:26,12:16)] 
...
# forest_plot <- forest_from_table(
#   df2plot = df2plot,
#   title = " Year 1 severe complete-case: multivariable regression model",
#   univariate = FALSE, imputed_output = FALSE)
# 
# filename <-paste(output_dir, "severe_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
# save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- severe_imputed_multivariable_regression_table[-c(1:6,22:27,34:39,40:42,56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
forest_plot <- forest_from_table(
  df2plot = df2plot,
  title = " Year 1 severe imputed: multivariable regression model",
  univariate = FALSE, imputed_output = TRUE)

filename <-paste(output_dir, "severe_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
...
```{r warning=FALSE}
variables2test <- names(original_psoriasis_data_mild)[c(3:11,23:24,25:26,12:16)]
...
``` {r}
variables2test <- names(imp_parallel_mild$data)[c(3:11,23:24,25:26,12:16)]
...
forest_plot <- forest_from_table(
  df2plot = df2plot[c(1:one_half),],
  title = " Year 1 mild complete-case: multivariable regression model",
  univariate = FALSE, imputed_output = FALSE)

filename <-paste(output_dir, "A_mild_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_mild_cc_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```

```{r}
df2plot <- mild_imputed_multivariable_regression_table[-c(56),] %>%
  dplyr::select(term, no, yes, estimate, p.value, conf.low, conf.high, p.bon, p.bh)
...
filename <-paste(output_dir, "A_mild_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
...
filename <-paste(output_dir, "B_mild_imp_multivariable_forest_plot_infection_antibiotic_year_1.jpeg", sep = "/")
save_forest_plot(forest_plot, filename)
```
...
filename <-paste(output_dir,"infection_and_antibiotic_year_1.xlsx", sep = "/")
writexl::write_xlsx(dataframes2save_list, path = filename)

# save first imputation instance
filename <-paste(output_dir,"first_imputation_instance_infection_and_antibiotic_year_1.xlsx", sep = "/")
writexl::write_xlsx(imputed_psoriasis_data_full, path = filename)
```

save mids objects for year-1 imputations
```{r}
mids2save <- to_vec(for (t in names(which(unlist(eapply(.GlobalEnv,is.mids))))) if(grepl("imp_",t)) t)

for(imd in mids2save){
  saveRDS(get(imd), file = paste0(output_dir, "/",imd,"_year_1.rds"))
}
```
...
write.table(
  ...
  file = paste0(
    output_dir,
    "/cases_other_infection_detection_age_year_1.csv",
    collapse = ""
  ),
  row.names = FALSE,
  quote = FALSE,
...
features2test <- names(original_psoriasis_data_full)[c(3:16,22:24)]
...
writexl::write_xlsx(
  ...
  path = paste0(output_dir, "/mean_onsetage_table_year_1.xlsx")
  )

writexl::write_xlsx(
  x = t_test_table_all, 
  path = paste0(output_dir, "/t_test_onsetage_association_year_1.xlsx")
  )

writexl::write_xlsx(
  x = wilcox_table_all, 
  path = paste0(output_dir, "/wilcox_test_onsetage_association_year_1.xlsx")
  )
```
...
original data
```{r}
predictors <- names(original_psoriasis_data_full)[c(3:11,23:24,25:26,12:16)]
...
```{r}
predictors <- names(original_psoriasis_data_post_puberty)[c(3:11,23:24,25:26,12:16)]
...
```{r}
predictors <- names(original_psoriasis_data_pre_puberty)[c(3:11,23:24,25:26,12:16)]
...
(c) girls-only data
```{r}
predictors <- names(original_psoriasis_data_girls)[c(3:11,23:24,25:26,12:16)]
...
(d) boys-only data
```{r}
predictors <- names(original_psoriasis_data_boys)[c(3:11,23:24,25:26,12:16)]
...
```{r}
predictors <- names(original_psoriasis_data_mild)[c(3:11,23:24,25:26,12:16)]
...
```{r}
predictors <- names(original_psoriasis_data_severe)[c(3:11,23:24,25:26,12:16)]
...
flextable::save_as_docx(
  ...
  path = paste0(outdir, "/year_1_firth_and_evalue_tables.docx"),
